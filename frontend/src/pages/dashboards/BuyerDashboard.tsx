...
function StatCard({ icon, label, value, loading }: { icon: string; label: string; value?: string; loading: boolean }) {
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: '20px 18px', display: 'flex', flexDirection: 'column', gap: 8 }}>
      <span style={{ fontSize: 26 }}>{icon}</span>
      <div style={{ fontSize: 11, color: 'var(--muted)', letterSpacing: '.05em', textTransform: 'uppercase', fontWeight: 600 }}>{label}</div>
      {loading ? <Skel h={28} r={6} /> : <div style={{ fontSize: 24, fontWeight: 800, fontFamily: 'var(--font-display)' }}>{value ?? '—'}</div>}
    </div>
  );
}
...
function OrderCard({ o }: { o: Order }) {
  ...
  return (
    <div style={{ background: 'var(--card-alt)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, display: 'flex', gap: 14 }}>
      <span style={{ fontSize: 28, flexShrink: 0 }}>{o.emoji}</span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: 14, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{o.product}</div>
        <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 3 }}>{o.seller} · {o.date}</div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, alignItems: 'center' }}>
          <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.06em', padding: '4px 10px', borderRadius: 999, color: s.color, background: s.bg }}>{s.label.toUpperCase()}</span>
          <span style={{ fontWeight: 800, fontSize: 14 }}>{fmt(o.amount)}</span>
        </div>
      </div>
    </div>
  );
...
function ProductCard({ p, onAdd }: { p: Product; onAdd: (id: string) => Promise<boolean> }) {
  ...
  return (
    <div style={{ background: 'var(--card-alt)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      {/* Image or emoji header */}
      <div style={{ height: 160, background: 'var(--card)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
        {p.image
      ...
      <div style={{ padding: '12px 14px 14px', display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3 }}>{p.seller} · {p.category}</div>
        </div>
        {p.rating > 0 && <div style={{ fontSize: 12, color: 'var(--muted)' }}>★ {p.rating} ({p.soldCount} sold)</div>}
        ...
        <div style={{ fontSize: 11, color: p.stock > 0 ? 'var(--muted)' : '#ef4444' }}>{p.stock > 0 ? `${p.stock} in stock` : 'Out of stock'}</div>
        <button disabled={!p.inStock || adding} onClick={async () => { setAdding(true); await onAdd(p.id); setAdding(false); }}
          style={{ padding: '9px 0', borderRadius: 10, border: 'none', cursor: p.inStock && !adding ? 'pointer' : 'not-allowed', background: p.inStock ? 'var(--accent)' : 'var(--border)', color: p.inStock ? '#fff' : 'var(--muted)', fontWeight: 700, fontSize: 12, fontFamily: 'var(--font-body)', opacity: adding ? 0.7 : 1, marginTop: 'auto' }}>
...
function CheckoutModal({ items, total, onClose, onSuccess }: { items: CartItem[]; total: number; onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.78)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
          ...
          {step === 'details' && <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: 20 }}>✕</button>}
        </div>

        {step === 'details' && (
          <>
            <div style={{ background: 'var(--card-alt)', borderRadius: 12, padding: 16, marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 12 }}>Order Summary</div>
              {items.map(i => (
                <div key={i.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <ProductThumb image={i.image} emoji={i.emoji} size={36} />
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 500 }}>{i.name}</div>
                      <div style={{ fontSize: 11, color: 'var(--muted)' }}>Qty {i.quantity} · {i.sellerName}</div>
                    </div>
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700 }}>{fmt(i.price * i.quantity)}</span>
                </div>
              ))}
              <div style={{ borderTop: '1px solid var(--border)', marginTop: 12, paddingTop: 12, display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontWeight: 700 }}>Total</span>
                <span style={{ fontWeight: 800, fontSize: 16, color: 'var(--accent)' }}>{fmt(total)}</span>
              </div>
            </div>
            ...
            {[['Delivery Address','delivery_address','text','e.g. 123 Moi Avenue, Nairobi'],['M-Pesa Phone','phone','tel','e.g. 0712 345 678']].map(([lbl,key,type,ph]) => (
              <div key={key} style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--muted)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.05em' }}>{lbl}</label>
            ...
            <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 18, display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={onClose} style={{ flex: 1, padding: 12, borderRadius: 10, border: '1px solid var(--border)', background: 'transparent', color: 'var(--muted)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 600 }}>Cancel</button>
              <button onClick={handlePay} style={{ flex: 2, padding: 12, borderRadius: 10, border: 'none', background: '#22c55e', color: '#fff', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        {step === 'waiting' && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 800, marginBottom: 10 }}>Enter Your M-Pesa PIN</h3>
            ...
            <button onClick={() => { stopPoll(); setStep('details'); setError(''); }} style={{ marginTop: 20, padding: '10px 24px', borderRadius: 10, border: '1px solid var(--border)', background: 'transparent', color: 'var(--muted)', cursor: 'pointer', fontFamily: 'var(--font-body)', fontSize: 13 }}>Cancel</button>
        ...
        {step === 'done' && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            <div style={{ fontSize: 56, marginBottom: 16 }}>🎉</div>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 800, marginBottom: 10 }}>Payment Successful!</h3>
            <p style={{ fontSize: 14, color: 'var(--muted)', lineHeight: 1.7, marginBottom: 16 }}>Your payment of <strong style={{ color: 'var(--text)' }}>{fmt(total)}</strong> was confirmed.</p>
            {mpesaCode && (
              <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 10, padding: '12px 20px', display: 'inline-block', marginBottom: 16 }}>
                <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>M-PESA RECEIPT</div>
                <div style={{ fontFamily: 'monospace', fontSize: 20, fontWeight: 800, color: '#22c55e', letterSpacing: 2 }}>{mpesaCode}</div>
              </div>
            )}
  ...
  return (
    <>
      <style>{`
        ...
        :root{--bg:#0d0f14;--card:#141720;--card-alt:#1a1e2b;--border:#1e2330;--text:#eef0f6;--muted:#6b7280;--accent:#6c63ff;--accent2:#ff6584;--sidebar-w:240px;--topbar-h:64px;--nav-h:62px;--font-display:'Syne',sans-serif;--font-body:'DM Sans',sans-serif;}
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .card-title{font-family:var(--font-display);font-size:16px;font-weight:700;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;}
        ...
        .search-box{width:100%;padding:14px 48px;background:var(--card-alt);border:1px solid var(--border);border-radius:14px;color:var(--text);font-family:var(--font-body);font-size:15px;outline:none;transition:border-color .2s;}
        .search-box::placeholder{color:var(--muted);}
        .search-box:focus{border-color:var(--accent);}
        .search-icon{position:absolute;left:16px;top:50%;transform:translateY(-50%);font-size:18px;pointer-events:none;}
        .search-clear{position:absolute;right:14px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--muted);font-size:18px;}
        .suggests{position:absolute;top:calc(100% + 6px);left:0;right:0;background:var(--card);border:1px solid var(--border);border-radius:12px;z-index:50;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.4);}
        .suggest-item{padding:11px 16px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;transition:background .12s;}
        ...
        .pages{display:flex;gap:6px;align-items:center;justify-content:center;padding-top:8px;}
        ...
        .qty-btn{width:28px;height:28px;border-radius:8px;border:1px solid var(--border);background:var(--card-alt);color:var(--text);cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center;font-weight:700;}
        ...
        @media(max-width:900px){
          ...
          .mob-btn{flex:0 0 auto;min-width:52px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;border:none;background:transparent;cursor:pointer;color:var(--muted);font-family:var(--font-body);font-size:9px;font-weight:600;position:relative;transition:color .15s;padding:0 8px;}
          .mob-btn.on{color:var(--accent);}
          .mob-btn .bi{font-size:20px;line-height:1;}
          .mob-badge{position:absolute;top:4px;right:4px;background:var(--accent2);color:#fff;font-size:8px;font-weight:800;border-radius:999px;min-width:15px;height:15px;display:flex;align-items:center;justify-content:center;padding:0 3px;}
        }
      `}</style>
      ...
      <div className="bd">
        ...
        <aside className="sidebar">
          <div className="sidebar-logo">
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 22 }}>Swift<span style={{ color: 'var(--accent)' }}>Mall</span></div>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>Buyer Portal</div>
          </div>
          <nav>
            {navItems.map(n => (
          ...
          <div className="sidebar-user">
            <div className="avatar" style={{ width: 38, height: 38, fontSize: 13 }}>{user?.initials ?? '?'}</div>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.name ?? '—'}</div>
              <div style={{ fontSize: 11, color: 'var(--accent)' }}>Buyer</div>
            </div>
          </div>
        </aside>
        ...
        <div className="main">
          <header className="topbar">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <button onClick={() => goTab('cart')} style={{ position: 'relative', background: 'var(--card-alt)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 14px', cursor: 'pointer', fontSize: 18 }}>
                🛒
                {itemCount > 0 && <span style={{ position: 'absolute', top: -6, right: -6, background: 'var(--accent2)', color: '#fff', fontSize: 9, fontWeight: 800, borderRadius: 999, minWidth: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>{itemCount > 9 ? '9+' : itemCount}</span>}
              </button>
          ...
          <main className="content">
            ...
            {tab === 'overview' && (
              <>
                <div>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 800 }}>{user ? `Hi, ${user.name.split(' ')[0]} 👋` : 'Welcome back 👋'}</div>
                  <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 6 }}>Here's what's happening with your account today.</div>
                </div>
                ...
                <div className="card">
                  ...
                  {ordersLoading ? [1,2,3].map(i => <Skel key={i} h={80} />) : orders.length === 0 ? <Empty icon="📦" msg="No orders yet. Start shopping!" /> : <div className="order-list">{orders.slice(0,3).map(o => <OrderCard key={o.id} o={o} />)}</div>}
                </div>
                <div style={{ textAlign: 'center', paddingBottom: 16 }}>
                  <button onClick={() => goTab('search')} style={{ padding: '14px 36px', borderRadius: 14, border: 'none', background: 'var(--accent)', color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>🛍 Browse & Shop</button>
                </div>
              </>
            )}
            ...
            {tab === 'search' && (
              <>
                    ...
                    <div className="suggests">
                      {suggestions.map((s, i) => (
                        <div key={i} className="suggest-item" onClick={() => { setQuery(s.name); setSuggestions([]); search(s.name); }}>
                          <div><div style={{ fontSize: 13, fontWeight: 500 }}>{s.name}</div><div style={{ fontSize: 11, color: 'var(--muted)' }}>{s.category}</div></div>
                          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent)' }}>{fmt(s.price)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {results && <span style={{ fontSize: 12, color: 'var(--muted)' }}>{results.total} result{results.total !== 1 ? 's' : ''}</span>}
                </div>
                <div className="card">
                  {searchLoading
                    ? <div className="grid-3">{[1,2,3,4,5,6].map(i => <Skel key={i} h={300} r={14} />)}</div>
                    ...
                    : <>
                        ...
                        {results.pages > 1 && (
                          <div className="pages" style={{ marginTop: 24 }}>
            ...
            {tab === 'cart' && (
              <div className="card">
                <div className="card-title">
                  My Cart {cartItems.length > 0 && <span style={{ fontSize: 13, color: 'var(--muted)', fontWeight: 400 }}>({itemCount} item{itemCount !== 1 ? 's' : ''})</span>}
                  {cartItems.length > 0 && <button className="pill danger" onClick={clearCart}>Clear</button>}
                </div>
                  ...
                  <>
                    {cartItems.map(item => (
                      <div key={item.id} className="cart-item">
                        {/* Image or emoji */}
                        <ProductThumb image={item.image} emoji={item.emoji} size={52} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 600, fontSize: 14 }}>{item.name}</div>
                          <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>{item.sellerName}</div>
                          <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--accent)', marginTop: 4 }}>{fmt(item.price)}</div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                          ...
                          <span style={{ fontSize: 13, fontWeight: 700 }}>{fmt(item.price * item.quantity)}</span>
                          <button onClick={() => removeCartItem(item.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontSize: 12 }}>Remove</button>
                        </div>
                      </div>
                    ))}
                    <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
                      <div>
                        <div style={{ fontSize: 12, color: 'var(--muted)' }}>Total</div>
                        <div style={{ fontSize: 28, fontWeight: 900, fontFamily: 'var(--font-display)', color: 'var(--accent)' }}>{fmt(cartTotal)}</div>
                      </div>
                      <button onClick={() => setShowCheckout(true)} style={{ padding: '14px 28px', borderRadius: 12, border: 'none', background: '#22c55e', color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                        📱 Pay with M-Pesa
                      </button>
                    </div>
                  </>
            ...
            {tab === 'orders' && (
              <div className="card">
                ...
                {ordersLoading ? [1,2,3].map(i => <Skel key={i} h={80} />) : orders.length === 0 ? <Empty icon="📦" msg="No orders found." /> : <div className="order-list">{orders.map(o => <OrderCard key={o.id} o={o} />)}</div>}
              </div>
            )}
            ...
            {tab === 'wishlist' && (
              <div className="card">
                ...
                {wishLoading ? <div className="grid-2">{[1,2,3,4].map(i => <Skel key={i} h={260} r={14} />)}</div>
                  ...
                  : <div className="grid-2">
                      {wishItems.map(item => {
                        ...
                        return (
                          <div key={item.id} style={{ background: 'var(--card-alt)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                            {/* Image or emoji */}
                            <div style={{ height: 130, background: 'var(--card)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                              {item.image
                            ...
                            <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
                              ...
                              <div style={{ display: 'flex', gap: 6, alignItems: 'baseline' }}>
                                <span style={{ fontWeight: 800, fontSize: 14 }}>{fmt(item.price)}</span>
                                {disc > 0 && <span style={{ fontSize: 11, color: 'var(--muted)', textDecoration: 'line-through' }}>{fmt(item.originalPrice)}</span>}
                              </div>
                              <div style={{ display: 'flex', gap: 8, marginTop: 'auto' }}>
                                <button disabled={!item.inStock} onClick={() => item.inStock && addToCart(item.id)}
                                  style={{ flex: 1, padding: '9px 0', borderRadius: 10, border: 'none', cursor: item.inStock ? 'pointer' : 'not-allowed', background: item.inStock ? 'var(--accent)' : 'var(--border)', color: item.inStock ? '#fff' : 'var(--muted)', fontWeight: 700, fontSize: 12, fontFamily: 'var(--font-body)' }}>
                                  {item.inStock ? '🛒 Add to Cart' : 'Out of Stock'}
                                </button>
                                <button onClick={() => removeWish(item.id)} style={{ padding: '9px 12px', borderRadius: 10, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', color: 'var(--muted)', fontSize: 14 }}>🗑</button>
                              </div>
                            </div>
                          </div>
                        );
            ...
            {tab === 'notifications' && (
              <div className="card">
                ...
                {notifsLoading ? [1,2,3].map(i => <Skel key={i} h={64} />) : notifications.length === 0 ? <Empty icon="🔔" msg="You're all caught up!" /> : (
                  notifications.map(n => (
                    <div key={n.id} className="notif-item">
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: n.read ? 'var(--border)' : 'var(--accent)', marginTop: 6, flexShrink: 0 }} />
                      <div style={{ flex: 1 }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.06em', padding: '3px 9px', borderRadius: 999,
              ...
              <>
                <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--accent)', marginTop: 4 }}>Buyer Account</div>
                  </div>
                </div>
                <div className="card">
